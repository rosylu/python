...
def build_parser():
    ...
    parser.add_argument('--overwrite', action='store_true', dest='overwrite',
            help='write file even if there is already a file with that name', default=True)
    return parser
...
def main():
    ...
    for i in range(len(style_images)):
        ...
        shape_ratio = style_scale * target_shape[1] / style_images[i].shape[1]
        style_images[i] = cv2.resize(style_images[i], (0,0), fx=shape_ratio, fy =shape_ratio)
    ...
    for iteration, image, loss_vals in stylize(
        ...
        if (image is not None) and (options.checkpoint_output is not None):
            print(fmt_imsave(options.checkpoint_output, iteration))
